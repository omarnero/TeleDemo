...
const LocationInputFrom: FC<Props> = ({
	...
	const renderPanelContent = () => {
		return (
			<div className="hiddenScrollbar flex space-x-4 overflow-x-auto">
				{data.map(item => {
					const isActive = item.name === value;
					return (
						<div
							key={item.id}
							className="group min-w-[130px] flex-1 cursor-pointer"
							onClick={() => {
								setValue(item.name);
						...
						>
							...
							/>
							<span className="mt-1.5 block text-black">{item.name}</span>
						</div>
					);
				})}
			</div>
