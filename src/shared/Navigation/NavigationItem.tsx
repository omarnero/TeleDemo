...
const NavigationItem: FC<NavigationItemWithRouterProps> = ({ menuItem }) => {
	...
	const renderMegaMenu = (menu: NavItemType) => {
		...
		return (
			<Popover
				as="li"
				className={`menu-item menu-megamenu space-x-2 ${classPopover} `}
		...
		return (
			<li key={item.id}>
				<NavLink
					...
					className={({ isActive }) =>
						`inline-flex items-center rounded py-1 px-2 font-normal text-neutral-6000 hover:bg-neutral-100 hover:text-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-800 dark:hover:text-neutral-200 `
					}
					to={item.href || ""}
				>
					{t(item.name)}
				...
				{() => (
					<>
						...
						>
							<Popover.Panel
								static
								className="sub-menu nc-will-change-transform absolute left-0 z-10 w-56 transform pt-3"
							>
				...
				{() => (
					<>
						...
						>
							<Popover.Panel
								static
								className="sub-menu absolute top-0 left-full z-10 w-56 pl-2"
							>
								<ul className="relative grid space-y-1 rounded-lg bg-white py-4 text-sm shadow-lg ring-1 ring-black ring-opacity-5 dark:bg-neutral-900 dark:ring-white dark:ring-opacity-10">
									{item.children?.map(i => {
										if (i.type) {
	...
	const renderDropdownMenuNavlink = (item: NavItemType) => {
		return (
			<NavLink
				...
				rel="noopener noreferrer"
				className={({ isActive }) =>
					`flex items-center rounded-md py-2 px-4 font-normal text-neutral-6000 hover:bg-neutral-100 hover:text-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-800 dark:hover:text-neutral-200 ${
	...
	switch (menuItem.type) {
		...
		default:
			return <li className="menu-item mx-2">{renderMainItem(menuItem)}</li>;
	}
};
